// Copy of ../../../functions/utils/webpush.ts kept in sync manually.
// Web Push (RFC 8291, aes128gcm) + VAPID (RFC 8292) for Cloudflare Workers.
...
async function signVapidJwt(
  ...
  const jwk = {
    kty: "EC",
    crv: "P-256",
    d: bytesToB64url(vapidPrivate),
    x: bytesToB64url(vapidPublic.slice(1, 33)),
    y: bytesToB64url(vapidPublic.slice(33, 65)),
    ext: true,
  };
  const key = await crypto.subtle.importKey(
    "jwk",
